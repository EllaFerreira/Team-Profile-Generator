function renderCards(teamMembers) {
  ...
  for (var i = 0; i < teamMembers.length; i++) {
    ...
    if (role === "Engineer") {
      card += `           <li class="list-items"><span>GitHub: </span><a href="https://github.com/${teamMembers[i].gitHub}" target='_blank'>${teamMembers[i].gitHub}</a></li>
        </ul>
      </div>
    </div>
  </div>
...
`;
    ...
    if (role === "Intern") {
      card += `           <li class="list-items"><span class="span-li">School: </span>${teamMembers[i].school}</li>
        </ul>
      </div>
    </div>
  </div>
